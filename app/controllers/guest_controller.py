...
class GuestController(BaseController):
	...
	def new_guest(self):
		...
		host_name = self.request.data.get('host_name')
		...
		guest = self.repo.new_guest(guest_name=guest_name, host_name=host_name, host_email=host_email, purpose=purpose, time_in=time_in, time_out=time_out, tag_no=tag_no)
		if guest:
